...
- Follow each language's normal naming conventions: camelCase for TypeScript identifiers and Svelte props, PascalCase for Svelte components and TypeScript types, and kebab-case for CSS classes and custom properties.
- Prefer the reusable components exported from `src/lib`: `GlassSurface`, `GlassButton`, `GlassSelect`, `SegmentedControl`, and `TextField`. Use the underlying classes only for specialized controls that a component cannot express cleanly.
- Keep `GlassButton` transparent at rest inside glass surfaces. Use `variant="standalone"` only without a material parent, `variant="primary"` for the main action, and `variant="danger"` for destructive actions.
...
- Use the shared emerald selected state for tabs, segmented controls, radio-button buttons, pressed controls, active labels, and selected navigation. Use `data-active="true"`, `aria-pressed="true"`, or `aria-selected="true"` with `glass-control`.

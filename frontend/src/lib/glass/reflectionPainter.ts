import { nearestPointOnRoundedEdge, transformPoint } from "./geometry";
import type { GlassTarget, ResolvedGlassOptions } from "./types";
...
export function paintTarget(
  ...
  const localPoint = nearestPointOnRoundedEdge(
    localPointer.x, localPointer.y, localRect, target.radii,
  );
  ...
  const proximity = Math.max(0, 1 - distance / options.glowRadius);
  ...
  paintInterior(
    context, path, canvasPoint, target.accent, strength, options.interiorRadius,
  );
  context.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0);
